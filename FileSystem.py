...
#dt_url = ['http://localhost:6998','http://localhost:6999','http://localhost:7000']#,'localhost:27020']
dt_url = []
Qr = 0
Qw = 0
...
class FileNode:
    ...
    def put(self,key,value):
	  ...
	  key = self.path+"&&"+key
 	  for x in dt_url:
	    ...
	    while not connected:
    		try:
	          rpc = xmlrpclib.Server(x)
                  temp1 = rpc.get(Binary(key))
         	  connected = True
		except:
         	  connected = False
            rpc = xmlrpclib.Server(x)
            rpc.put(Binary(key), Binary(pickle.dumps(value)), 6000)
          #print 'The error record', rpc.corrupt(Binary(key))
        else:
          for x in dt_url:
	    connected = False
	    while not connected:
	...
	flag_Qr_check= False
        if key == "data" :
          key = self.path+"&&"+key
	  while (flag_Qr_check == False):
		  print 'Flag start of while: ', flag_Qr_check
		  for x in dt_url:
			 connected = False
			 while not connected:
	    			 try:
		         ...
		         if 'value' in temp1:
		           append_value =pickle.loads(temp1['value'].data)
		           res_d.append(append_value) 
	 		   url_d[append_value]=x
		         else: 
		           res_d.append('NULL')
			   url_d['NULL']=x
		  print 'Res list is' , res_d
		  print 'URL dict is', url_d
		  if not res_d:
			print 'list is empty to do TMR'
			rpc = xmlrpclib.Server(dt_url[0])
			...
			k=Counter(res_d)
			res= max(k,key=k.get)
			res_min=min(k,key=k.get)
		        print 'Sorted TMR', k
		        print 'URL D', url_d
			print 'After TMR max', res
			print 'After TMR min', res_min
			rpc = xmlrpclib.Server(url_d[res_min])
	    		if (res != res_min):
	  		   if (res_min == 'NULL'):
				print 'Server Crash Error'
				res_sync = rpc.sync()
	 		   else:
				print 'Data Corruption Error'
				res_sync = rpc.sync_rec(Binary(key))
                #res_sync = rpc.sync()
		           print 'URL to correct', url_d[res_min]
			   print 'Qr and k[res]', Qr, k[res]
		           

			   if (k[res]>=int(Qr)):
			   	print 'Qr condition satisfied', Qr, k[res]
				flag_Qr_check = True
			   else:
				print 'Qr condition not satisfied', Qr, k[res]
				flag_Qr_check = False
				k.clear()
				url_d.clear()
				del res_d[:]
...
if __name__ == "__main__":
  ...
  global Qr, Qw
  if len(argv) < 8:
    print 'usage: %s <mountpoint>  Qr Qw <meta Server port> <data Server port 1> <data Server port 2> .. <data Server port n>' % argv[0]
    exit(1)
  url = argv[4]
  Qr = argv[2]
  Qw = argv[3]
  for x in range(5,len(argv)):
     dt_url.append(argv[x]) 
